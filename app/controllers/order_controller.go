...
func (r *OrderController) UpdateOrder(c *gin.Context) {
	...
	if file != nil {
		...
		if order.Gambar != "" {
			if err := os.Remove(order.Gambar); err != nil {
		...
		order.Gambar = filepath
	}

	order.NamaBarang = input.NamaBarang
	order.URL = input.URL
...
func (r *OrderController) ConfirmOrder(c *gin.Context) {
	id := c.Param("id")
	if err := database.DB.Model(&models.Order{}).Where("id = ?", id).Update("status", models.Terima).Error; err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"message": err.Error()})
		return
	}
