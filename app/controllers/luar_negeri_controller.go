...
func (r *LuarNegeriController) GetLuarNegeri(c *gin.Context) {
	var data []models.ProcessOrder

	result := database.DB.Preload("User").Preload("Barang").Where("process_orders.status = ?", "proses luar negeri").Find(&data)
	if result.Error != nil {
